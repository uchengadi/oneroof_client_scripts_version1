/*
 * File: app/controller/MemberNumberController.js
 *
...
Ext.define('cobuy.controller.MemberNumberController', {
    ...
    views: [
        'MembershipNumberMainpanel',
        'GenerateMembershipNumberWin'
    ],

    refs: [
        {
            ref: 'allMemberNumberGrid',
            selector: 'membershipnumbermainpanel grid'
        }
    ],

    onGenerateMemberNumberButtonClick: function(button, e, eOpts) {
        var grid = this.getAllMemberNumberGrid(),
        		record = grid.getSelectionModel().getSelection();

        		if(record[0]) {
        			var editWin = Ext.create('cobuy.view.GenerateMembershipNumberWin');
        			...
        			editWin.show();
        				...
        				Ext.Msg.alert('Error!', 'Please Select a Member to generate his/her Membership Number');
        		}
    },

    onSaveGeneratedMemberNumberButtonClick: function(button, e, eOpts) {
        var win = button.up('window'),
        			formPanel = win.down('form'),
        			store = this.getAllMemberNumberGrid().getStore();

        			if(formPanel.getForm().isValid()){
        				formPanel.getForm().submit({
        					clientValidation: true,
        					url: '/cobuy/index.php?r=Members/GenerateNewMemberNumber',
                           //success or failure
    ...
    onMembershipNumberGridpanelItemDblClick: function(dataview, record, item, index, e, eOpts) {
        this.onGenerateMemberNumberButtonClick();
    },

    onMembershipNumberGenerationPanelRender: function(component, eOpts) {
        component.down('grid').getStore().load();
    },

    init: function(application) {
        this.control({
            "membershipnumbermainpanel button#generateBtn": {
                click: this.onGenerateMemberNumberButtonClick
            },
            "generatemembershipnumberwin button#generateBtn": {
                click: this.onSaveGeneratedMemberNumberButtonClick
            },
            "membershipnumbermainpanel grid": {
                itemdblclick: this.onMembershipNumberGridpanelItemDblClick
            },
            "membershipnumbermainpanel": {
                render: this.onMembershipNumberGenerationPanelRender
            }
        });
    }
