/*
 * File: app/controller/NonStaffMembersController.js
 *
...
Ext.define('cobuy.controller.NonStaffMembersController', {
    ...
    refs: [
        {
            ref: 'allNonStaffMembersGrid',
            selector: 'membermainpanel grid'
        }
    ],

    onAddNonStaffMemberButtonClick: function(button, e, eOpts) {
        var win = Ext.create('cobuy.view.NewMemberWin');

        win.show();
    },

    onEditNonStaffMembersButtonClick: function(button, e, eOpts) {
        var grid = this.getAllNonStaffMembersGrid(),
        		record = grid.getSelectionModel().getSelection();

        		if(record[0]) {
        			var editWin = Ext.create('cobuy.view.EditMemberWin');
        			...
        			editWin.show();
        				...
        				Ext.Msg.alert('Error!', 'Please Select a Non Staff Member to edit');
        		}
    },

    onDeleteNonStaffMemberButtonClick: function(button, e, eOpts) {
        var grid = this.getAllNonStaffMembersGrid();
    ...
    onSaveNewNonStaffMemberButtonClick: function(button, e, eOpts) {
        var win = button.up('window'),
        			formPanel = win.down('form'),
        			store = this.getAllNonStaffMembersGrid().getStore();

        			if(formPanel.getForm().isValid()){
        				formPanel.getForm().submit({
        					clientValidation: true,
        					url: '/cobuy/index.php?r=Members/AddNewNonStaffMember',
                           //success or failure
    ...
    onSaveEdittedNonStaffMemberButtonClick: function(button, e, eOpts) {
        var win = button.up('window'),
        			formPanel = win.down('form'),
        			store = this.getAllNonStaffMembersGrid().getStore();
        			...
        			if(formPanel.getForm().isValid()){
        				formPanel.getForm().submit({
        					clientValidation: true,
        					url: '/cobuy/index.php?r=Members/UpdateNonStaffMember',
                           //success or failure
        					success: function(form, action) {
        						var result = action.result;
        						if(result.success) {
    ...
    onNonStaffMemberGridpanelItemDblClick: function(dataview, record, item, index, e, eOpts) {
        this.onEditNonStaffMembersButtonClick();
    },

    onMemberPanelRender: function(component, eOpts) {
        component.down('grid').getStore().load();
    },

    init: function(application) {
        this.control({
            "membermainpanel button#addBtn": {
                click: this.onAddNonStaffMemberButtonClick
            },
            "membermainpanel button#editBtn": {
                click: this.onEditNonStaffMembersButtonClick
            },
            "membermainpanel button#deleteBtn": {
                click: this.onDeleteNonStaffMemberButtonClick
            },
            "newmemberwin button#addBtn": {
                click: this.onSaveNewNonStaffMemberButtonClick
            },
            "editmemberwin button#addBtn": {
                click: this.onSaveEdittedNonStaffMemberButtonClick
            },
            "membermainpanel grid": {
                itemdblclick: this.onNonStaffMemberGridpanelItemDblClick
            },
            "membermainpanel": {
                render: this.onMemberPanelRender
            }
        });
    }
